...
import { createModule, deleteModule, getModule, updateModule } from "@/lib/drizzle_functions";
...
const handleGet = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    ...
    const {module_id}  = getModulesReqSchema.parse(req.query)
    const data = await getModule(module_id)
...
const handlePut = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    ...
    const data = await updateModule(payload)
    res.status(200).json(data);
  } catch (error) {
    const handler = new ServerErrorHandler(error);
    handler.log();
